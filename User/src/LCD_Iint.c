...
* 功    能：将屏幕填充成指定的颜色，如清屏，则填充 0xffff（白色）
* 入口参数：dat      填充值
* 出口参数：无
* 说    明：
* 调用方法：ili9320_Clear;
...
/* 水平线 */
void rt_hw_lcd_draw_hline(unsigned int colour, unsigned int x1, unsigned int x2, unsigned int y)
{
...
/* 垂直线 */
void rt_hw_lcd_draw_vline(unsigned int colour, unsigned int x, unsigned int y1, unsigned int y2)
{
    /* [5:4]-ID~ID0 [3]-AM-1垂直-0水平 */
