...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));	  //检测参数GPIOx
...
/**
  ...
  */
uint8_t GPIO_ReadInputDataBit(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)  //读数据1位
{
  ...
  if ((GPIOx->IDR & GPIO_Pin) != (uint32_t)Bit_RESET)	 //IDR--数据输入寄存器（可查看数据手册）
  {														 //ODR--数据输出寄存器
...
/**
  ...
  */
uint16_t GPIO_ReadInputData(GPIO_TypeDef* GPIOx) //读整个数据，即读整个IO口
{
  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
...
uint8_t GPIO_ReadOutputDataBit(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
  ...
  if ((GPIOx->ODR & GPIO_Pin) != (uint32_t)Bit_RESET)  //ODR--数据输入寄存器（可查看数据手册）
  {
    bitstatus = (uint8_t)Bit_SET;
  }
  else
...
/**
  ...
  * @retval None
  */
void GPIO_SetBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)  //对输出而言（可同时对多位操作）
{
  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
  assert_param(IS_GPIO_PIN(GPIO_Pin));
  
  GPIOx->BSRR = GPIO_Pin;  //BSRR--端口位设置寄存器（可查看数据手册）
}
...
/**
  ...
  * @retval None
  */
void GPIO_ResetBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)	 //对输出而言（可同时对多位操作）
{
  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
  assert_param(IS_GPIO_PIN(GPIO_Pin));
  
  GPIOx->BRR = GPIO_Pin;  //BRR--端口位复位寄存器（可查看数据手册）
}
...
/**
  ...
  * @retval None
  */
void GPIO_WriteBit(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, BitAction BitVal) //（一次只能对1位操作）
{
  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
  assert_param(IS_GET_GPIO_PIN(GPIO_Pin));
...
/**
  ...
  * @retval None
  */
void GPIO_PinRemapConfig(uint32_t GPIO_Remap, FunctionalState NewState)	 //第三方引脚复用配置
{
